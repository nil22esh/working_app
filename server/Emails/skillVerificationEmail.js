...
export const skillVerificationRequestEmail = async (user) => {
  try {
...
<div style="font-family: Arial, sans-serif; text-align: center; padding: 20px; background-color: #f4f4f4;">
  <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
	...
	<div style="padding: 20px;">
	  ...
	  <p style="font-size: 16px; color: #333333;">
		<b>Your request has been successfully subbmitted for skill verification!</b> Please wait once completed you will receive an Email!
	  </p>
...
`;
    ...
    const mailOptions = {
      ...
      to: user.email,
      subject: "Skill Verification Request Subbmitted Successfully!",
      html: emailHtml,
    };

    await transporter.sendMail(mailOptions);
    console.log("Skill Verification Email sent successfully!");
  } catch (error) {
    console.log(`Error: ${error.message}`);
    throw new Error("Could not send skill request email");
  }
};
